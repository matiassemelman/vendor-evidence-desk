...
export const META = {
  ...
  costModel: ["The release records token usage per completed live run; five synthetic cases do not establish a representative production cost."],
  deepDive: claimTable,
...
export const NODES = [
  ...
  },
  {
    ...
    how: "<code>lib/ai/worker.ts</code> uses OpenAI Responses with strict JSON Schema and <code>maxRetries: 0</code>; <code>lib/ai/run.ts</code> runs the three workers concurrently.",
  ...
  },
  {
    ...
    how: "The judge call in <code>evals/run.ts</code> uses strict output, one batch and calibrated anchors; deterministic route assertions remain primary.",
