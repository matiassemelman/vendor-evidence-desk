...
export async function insertRevision(revision: CaseRevision) {
  ...
  if (!target) { local.set(revision.revisionId, structuredClone(revision)); return false; }
  const { db, table } = target; await db`insert into ${table} (revision_id, case_id, lineage_id, parent_revision_id, input_hash, decision_digest, analysis, lifecycle, current_state, source) values (${revision.revisionId}, ${revision.caseId}, ${revision.lineageId}, ${revision.parentRevisionId}, ${revision.inputHash}, ${revision.decisionDigest}, ${JSON.stringify(analysis(revision))}::jsonb, ${JSON.stringify(revision.lifecycle)}::jsonb, 'current', 'analysis')`;
  return true;
}
export async function getRevision(revisionId: string) {
  const target = connection();
  if (!target) return local.get(revisionId) ? structuredClone(local.get(revisionId)!) : undefined;
  const { db, table } = target, rows = await db`select analysis, lifecycle, current_state from ${table} where revision_id = ${revisionId}`;
...
export async function createSuccessor(parentRevisionId: string, child: CaseRevision) {
  const target = connection(), db = target?.db, parent = await getRevision(parentRevisionId);
  ...
  const supersededAt = new Date().toISOString(), table = target!.table;
  const result = await db`with superseded as (
    update ${table} set current_state = 'superseded', lifecycle = lifecycle || ${JSON.stringify({ validity: "superseded", supersededAt })}::jsonb
    where revision_id = ${parentRevisionId} and lineage_id = ${child.lineageId} and current_state = 'current' returning revision_id
  ), inserted as (
    insert into ${table} (revision_id, case_id, lineage_id, parent_revision_id, input_hash, decision_digest, analysis, lifecycle, current_state, source)
...
export async function approveAndExport(revisionId: string, lineageId: string, decisionDigest: string, selected: string, reason: string, receipt: ApprovalReceipt) {
  const target = connection(), db = target?.db, current = await getRevision(revisionId);
  ...
  const lifecycle = { ...current.lifecycle, approval: { selected, reason, approvedAt: receipt.approvedAt, receiptId: receipt.receiptId, decisionDigest }, export: receipt.export }, table = target!.table;
  const updated = await db`update ${table} set lifecycle = ${JSON.stringify(lifecycle)}::jsonb, approval_receipt = ${JSON.stringify(receipt)}::jsonb, export_receipt = ${JSON.stringify(receipt.export)}::jsonb where revision_id = ${revisionId} and lineage_id = ${lineageId} and decision_digest = ${decisionDigest} and current_state = 'current' and approval_receipt is null returning revision_id`;
